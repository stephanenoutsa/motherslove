...
public class MyHivService extends Service {
        ...
        final Handler handler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                try {
                    ...
                    int received = dbHandler.getHivReceived();
                    ...
                    if (diff >= 90 && diff < 266) {
                        if (received == 1) {
                            ...
                            notification.setContentText(getString(R.string.hiv_notif1));
                            ...
                            String nmessage = getString(R.string.hiv_notif1);
                    ...
                    // Fire 2nd HIV notification
                        ...
                        if (received == 2) {
                            ...
                            notification.setTicker(ticker); // Sets the text displayed when notification is received
                            notification.setWhen(System.currentTimeMillis());
                            notification.setContentText(getString(R.string.hiv_notif2));
                            int rand = r.nextInt(1000);
                            nm.notify(rand, notification.build());
                            ...
                            DateFormat f = new SimpleDateFormat("dd/MM/yyyy");
                            String nday = f.format(new Date().getTime());
                            String nmessage = getString(R.string.hiv_notif2);
                            notif = new Notification(nday, nmessage);
                            dbHandler.addNotification(notif);
                    ...
                    // Fire 3rd HIV notification
                    else if (diff >= 295 && diff < 300) {
                        if (received == 3) {
                            ticker = trimText(getString(R.string.hiv_notif3));
                            notification.setTicker(ticker); // Sets the text displayed when notification is received
                            notification.setWhen(System.currentTimeMillis());
                            notification.setContentText(getString(R.string.hiv_notif3));
                            int rand = r.nextInt(1000);
                            nm.notify(rand, notification.build());
                            ...
                            DateFormat f = new SimpleDateFormat("dd/MM/yyyy");
                            String nday = f.format(new Date().getTime());
                            String nmessage = getString(R.string.hiv_notif3);
                            notif = new Notification(nday, nmessage);
                            dbHandler.addNotification(notif);
                    ...
                    // Fire 4th HIV notification
                    else if (diff == 300) {
                        if (received == 4) {
                            ticker = trimText(getString(R.string.hiv_notif4));
                            notification.setTicker(ticker); // Sets the text displayed when notification is received
                            notification.setWhen(System.currentTimeMillis());
                            notification.setContentText(getString(R.string.hiv_notif4));
                            int rand = r.nextInt(1000);
                            nm.notify(rand, notification.build());
                            ...
                            DateFormat f = new SimpleDateFormat("dd/MM/yyyy");
                            String nday = f.format(new Date().getTime());
                            String nmessage = getString(R.string.hiv_notif4);
                            notif = new Notification(nday, nmessage);
                            dbHandler.addNotification(notif);
